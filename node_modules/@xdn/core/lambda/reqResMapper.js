...
function getXdnVersion() {
    return process.env[constants_1.XDN_ENV_VARIABLES.versionOverride] || require('../../package.json').version;
}
exports.default = (function (event, callback) {
    ...
    req.rawHeaders = [];
    req.headers = {};
    if (!isEmpty_1.default(event.multiValueQueryStringParameters)) {
        var qs_1 = querystring_1.stringify(event.multiValueQueryStringParameters);
        req.url += "?" + qs_1;
        req.query = querystring_1.parse(qs_1); // we stringify and reparse to ensure that query parameters with a single value are not provided as an array
    }
    ...
    req.connection = {};
    // req.secure is ExpressJS extension, set to false only when the protocol is HTTP
    // (so the default remains HTTPS which has historically been the only protocol we allowed)
    req.secure = req.getHeader(constants_1.HTTP_HEADERS.xXdnProtocol) !== 'http';
    // req.socket.encrypted is set to true by NodeJS when the protocol is HTTPS.
    req.socket = {
    ...
    // - Router when using `xdn run`
    // - Handler when using `xdn run --production`
    ...
    var onResEnd = function (callback, resolve) { return function (text) {
        ...
        if ((req.getHeader(constants_1.HTTP_HEADERS.acceptEncoding) || '').split(',').includes('gzip') &&
            res.getHeader(constants_1.HTTP_HEADERS.contentEncoding) == null) {
...
function logStatusCode(response) {
    prefixResponseHeader(response, constants_1.HTTP_HEADERS.xXdnStatus, "w=" + response.statusCode);
}
/**
...
function logExecutionTime(response, startTime) {
    ...
    prefixResponseHeader(response, constants_1.HTTP_HEADERS.xXdnT, "wt=" + lambdaTime);
}
/**
...
function logComponentVersion(response, version) {
    prefixResponseHeader(response, constants_1.HTTP_HEADERS.xXdnComponents, "w=" + version);
}
/**
